...
function SignUp () {
    ...
    const createUser = async (userObj) => {
        ...
        setLoading(true)
        const data = await callApi('createUser', 'post', userObj);
        setLoading(false);
        if (data) {
            msg.s('Account created successfully. Please login.')
                                ...
                                <div className="_sign_r8_sngl_all">
                                    <Form onFinish={createUser}>
                                        ...
                                        <div className="_sign_r8_btn">
                                            <Button disabled={loading} htmlType="submit" style={{padding: "8px 0px"}} type="_btn1">{loading ? 'Please wait...' :'Sign Up'}</Button>
                                        </div>
                                    </Form>
                                <div className="_signUp_card_btm">
                                    <div className="_signUp_card_acnt _dis_flex _dis_flex_cntr1">
                                        <p >Have an account?</p>
                                        <Link to="/login" style={{marginTop: "-16px"}}>Sign In</Link>
                                    </div>
                                </div>
